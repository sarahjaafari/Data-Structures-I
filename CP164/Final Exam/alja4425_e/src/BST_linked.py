...
class BST:
    ...
    def _are_adjacents_aux(self, parent, v1, v2):
        ...
        if parent is None:
            return False
        if (parent._left is not None and parent._left._value == v1 and parent._right is not None and parent._right._value == v2) or \
                (parent._left is not None and parent._left._value == v2 and parent._right is not None and parent._right._value == v1):
            return True
        if v1 < parent._value and v2 < parent._value:
            return self._are_adjacents_aux(parent._left, v1, v2)
        elif v1 > parent._value and v2 > parent._value:
            return self._are_adjacents_aux(parent._right, v1, v2)
        else:
            return False
    ...
    def _max_path_aux(self, node):
        ...
        if node is None:
            return [], 0
        left_path, left_length = self._max_path_aux(node._left)
        right_path, right_length = self._max_path_aux(node._right)
        if left_length > right_length:
            return [node._value] + left_path, left_length + 1
        else:
            return [node._value] + right_path, right_length + 1
